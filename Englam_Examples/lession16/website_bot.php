...
function describe_zipcode($zipcode)
    ...
    //<input type="hidden" name="session" 為了要把過濾的 hidden and session上面能帶雙引號，
    //所以這邊的start 有用到一些 \
    
    
    # Remove the "'s and "value=" text to reveal the session value
    //這個時候會回傳 value=1486637157 <-隨機
    ...
    #傳送 http post pack and 將data代入
    $form_result = http_post_form($target, $ref=$target, $data_array);
    
    /* 回傳的example
     	<table class="textblackbold10">
		<tr><td colspan="2" class="textblackbold12">Information about 55441</td></tr>
		<tr><td><b>City</b></td><td>MINNEAPOLIS</td></tr>
		<tr><td><b>County</b></td><td>HENNEPIN</td></tr>
		<tr><td><b>State</b></td><td>MN</td></tr>
		<tr><td><b>Latitude</b></td><td>+45.005804</td></tr>
		<tr><td><b>Longitude</b></td><td>-093.419323</td></tr>
		</table> 
     * */
